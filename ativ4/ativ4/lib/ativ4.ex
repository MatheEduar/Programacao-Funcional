defmodule Ativ4 do
  ...
  def tamanho_strings(ls) do
    map(ls, fn x -> String.length(x) end) 
  end
  ...
  def converte_para_inteiros(l) do
    map(l, fn x -> String.to_integer(x) end)
  end
  ...
  [101, 102, 103, 104, 105]
  """
  def soma_100_lista(l) do
    map(l, fn x -> x + 100 end)
  end
  ...
  """
  def adiciona_prefixo(l, pre) do
    map(l, fn x -> pre<>x end)
  end
  ...
  """
  def adiciona_sufixo(l, sufixo) do
    map(l, fn x -> x<>sufixo end)
  end
  ...
  """
  def and_lista(lb) do
    fold_right(lb, true, fn (x, s) -> x and s end)
  end
  ...
  """
  def or_lista(lb) do
    fold_right(lb, false, fn (x,s) -> x or s end)
  end
  ...
  def fold_left([head | tail], ini, fun) do
    fold_left(tail, fun.(ini, head), fun)
  end
  ...
  915
  """
  def pontuacao_final(ini, deducoes) do
    fold_left(deducoes, ini, fn (r,x) -> r - x end)
  end
  ...
  [1, 2, 3, 4, 7, 8, 9]
  """
  def concatena_listas(ll) do
    fold_right(ll, [], fn (x, r) -> x++r end)
  end
  ...
  def concat_strings_sep([head | tail], sep) do
    fold_left(tail, head, fn (x,n) -> x<>sep<>n end)
  end
  ...
  ["euforia", "Moria"]
  """
  def strings_que_contem(ls, meio) do
    filter(ls, fn x -> String.contains?(x, meio) end)
  end
  ...
  [1, 3, 5]
  """
  def par_3n_mais_1(l) do
    filter(l, fn x -> rem(x * 3 + 1, 2) == 0 end)
  end
end
